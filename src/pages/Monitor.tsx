...
const FleetMonitor = () => {
    ...
    const sendWhatsApp = () => {
        ...
        // Build message from customizable template (stored in localStorage)
        const template = localStorage.getItem('hd_whatsapp_template')
            || 'Hola {nombre}, sigue tu entrega en tiempo real aquí: {link}';
        const message = template
            .replace(/{nombre}/gi, lastCreatedTrip.clientName || 'Cliente')
            .replace(/{link}/gi, link)
            .replace(/{costo}/gi, `$${(lastCreatedTrip.cost || 0).toFixed(2)}`);

        // Multi-phone support: split by '&'
        const rawPhone = lastCreatedTrip.clientPhone || '';
        const phones = rawPhone.split('&').map((p: string) => p.trim()).filter(Boolean);

        let selectedPhone = '';
        if (phones.length > 1) {
            const options = phones.map((p: string, i: number) => `${i + 1}) ${p}`).join('\n');
            const choice = prompt(`Este cliente tiene ${phones.length} teléfonos:\n${options}\n\nEscribe el número de opción (1, 2, etc.):`);
            if (!choice) return;
            const idx = parseInt(choice) - 1;
            if (idx >= 0 && idx < phones.length) {
                selectedPhone = phones[idx].replace(/\D/g, '');
            } else {
                alert('Opción inválida');
                return;
            }
        } else if (phones.length === 1) {
            selectedPhone = phones[0].replace(/\D/g, '');
        }

        const waLink = selectedPhone
